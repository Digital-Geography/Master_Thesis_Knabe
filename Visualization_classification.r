...
#Cleaning and date manipulation     
    ...
    Meteo_pred <- Meteo_pred %>% dplyr::select(date, count, creation_y, prediction, prediction_exc)
...
##### Calculate classification thresholds based on percentiles #####
    ...
    # New df with probabilites of presence cells. sort descending and calculate the percentage of observations
      ...
      #define threshholds
      p25 <- ind_val %>% filter(abs(observations - 25) == min(abs(observations - 25))) %>% slice(1) %>% pull(Probability)
      ...
      #define threshholds
      p25_meteo <- ind_val_meteo %>% filter(abs(observations - 25) == min(abs(observations - 25))) %>% slice(1) %>% pull(Predicted)
...
###### For Habiat suitability ###
   theme_set(
    ...
    prob_reclass <- ggplot(ind_val, aes(x = observations, y = Probability)) +
      ...
      annotate("rect", xmin = 0, xmax = 100, ymin = p95, ymax = p25, alpha = 0.6, fill = "#FFDC00") +
      annotate("rect", xmin = 0, xmax = 100, ymin = p25, ymax = p0, alpha = 0.6, fill = "#FF0000") +
      geom_line(linewidth = 0.6) +
      geom_vline(xintercept = c(25, 95), linetype = "dotted", alpha = 0.6, color = "white", linewidth = 0.6) +
      geom_hline(yintercept = c(p95, p25), color = "white", alpha = 0.6, linewidth = 0.6) +
    ...
    reclass_matrix <- matrix(c(0, p95, 1, p95, p25, 2, p25, Inf, 3), ncol = 3, byrow = TRUE)
  ...
  ### Now the same for temporal model ###
    pred_reclass <- ggplot(ind_val_meteo, aes(x = observations, y = Predicted)) +
      ...
      annotate("rect", xmin = 0, xmax = 100, ymin = p95_meteo, ymax = p25_meteo, alpha = 0.6, fill = "#FFDC00") +
      annotate("rect", xmin = 0, xmax = 100, ymin = p25_meteo, ymax = p0_meteo, alpha = 0.6, fill = "#FF0000") +
      geom_line(linewidth = 0.6) +
      geom_vline(xintercept = c(25, 95), linetype = "dotted", alpha = 0.6, color = "white", linewidth = 0.6) +
      geom_hline(yintercept = c(p95_meteo, p25_meteo), color = "white", alpha = 0.6, linewidth = 0.6) +
  ...
  prob_medium_risk <- mean(Meteo_pred$prediction_exc >= p95_meteo & Meteo_pred$prediction_exc < p25_meteo, na.rm = TRUE) * 100 + prob_low_risk
  prob_high_risk <- mean(Meteo_pred$prediction_exc >= p25_meteo & Meteo_pred$prediction_exc <= p0_meteo, na.rm = TRUE) * 100 + prob_medium_risk  
...
# Apply classification threshholds to Meteo_pred and Suitability
  Meteo_pred <- Meteo_pred %>% mutate(
    classification = case_when(
      ...
      prediction_exc >= p95_meteo & prediction_exc < p25_meteo ~ 1, # Medium risk
...
#########################################
    ...
    for (i in 1:nlayers(abundance_stack_norm)) {
      ...
      plot(abundance_stack_norm[[i]], main = as.character(Meteo_pred_GIF$date[i]), 
           ...
           legend.args = list(text = "Risk", side = 3, line = 1.2, cex = 0.95, adj = 0.5, font = 2))
      dev.off()
      images[[i]] <- plot_file
    }
